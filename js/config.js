 ...
 *
 */
function config($routeProvider, $ocLazyLoadProvider, jwtOptionsProvider, $httpProvider, $middlewareProvider, usSpinnerConfigProvider) {
  $ocLazyLoadProvider.config({
    // Set to true if you want to see what and when is dynamically loaded
    debug: false
  });

  jwtOptionsProvider.config({unauthenticatedRedirectPath: '/login', whiteListedDomains: ['localhost']});
  ...
  $middlewareProvider.map({
    ...
    'everyone': [
      ...
      '$location',
      'jwtHelper',
      function everyoneMiddleware($cookieStore, $rootScope, $http, $location, jwtHelper) {
        if ($location.path() == '/login') {
          ...
          if ($rootScope.adminglob.currentUser) {
            if (jwtHelper.isTokenExpired($rootScope.adminglob.currentUser.token)) {
              ...
              $http.defaults.headers.common['authorization'] = $rootScope.adminglob.currentUser.token; // jshint ignore:line
              this.next();
            }
          } else {
...
angular.module('inspinia').config(config).run(function($rootScope, authManager, $cookieStore, AuthenticationService, $location) {
